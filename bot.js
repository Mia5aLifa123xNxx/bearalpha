...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'bearboy Community ©\'s translator',
...
client.on('message', message => {
    if (message.content.startsWith("._.bans")) {
...
client.on('guildCreate', guild => {
     ...
     .setURL('80k credits for link invite')
  .setDescription(`**
  New Server Add bearboy Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('80k credits for link invite')
  .setDescription(`**
  Server Kicked 3bearboy Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
...
client.on('message', message => {
                                if(!message.channel.guild) return;
                        if (message.content.startsWith('._.ping')) {
                            if(!message.channel.guild) return;
...
client.on('message', message => {
    if(!message.guild) return;
    if (message.author.bot) return;
      let id = message.author.id,prefix="._.";//البريفكس
...
if(message.content.startsWith(prefix + "slots")) {
  let slot1 = ['1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣', '6️⃣', '7️⃣', '8️⃣'];
...
client.on('guildMemberAdd', (member) => {
member.addRole(member.guild.roles.find('name', 'chekeing...')); /// m غير اسم الربته اذا تبي
});
...
const fetchVideoInfo = require("youtube-info");
const initcmd = '._.';
const botkahrba= ['536928110055260170']; // الايدي بتاعك انتا ووالادارة
...
// By : Kahrba. || تم التطوير من قبل كههربا
		case "p":
			if (args.length == 0 && queue.length > 0) {
				...
				message.reply("قائمة التشغيل فارغة الآن , ._.p [ واسم الاغنية ] or ._.yt [ ومصطلح البحث ] || لتشغيل والبحث عن الاغاني");
		...
		case "s":
			...
			if (queue.length === 1) {// By : Kahrba. || تم التطوير من قبل كههربا
				...
				description: " ⁉ || **__قائمة التشغيل فارغة الان , اكتب ._.p [اسم الاغنية] او ._.yt [اسم الاغنية]__**"
				}});// By : Kahrba. || تم التطوير من قبل كههربا
				dispatcher.end();
			} else {
		...
		case "pl":
			if (queue.length === 0) { // اذا لم تكن هناك اغاني في قائمة التشغيل , ف يبعت رسالة ان قائمة الشتغيل
						...
						message.channel.send({embed: {
				color: 3447003,
				description: " 😒 || **__قائمة التشغيل فارغة , ``اكتب : ._.p | ._.yt`` للبحث علي الاغاني__**"
				}});
			...
			} else if (args.length > 0 && args[0] == 'Mclr') {
				        let kahrba = message.guild.member(message.author).roles.find('name', 'Dj');
				if (args.length == 1) {
// By : Kahrba. || تم التطوير من قبل كههربا
						message.reply(` `);
					...
					songsQueue.splice(1);
						...
						message.reply(` `);
                             const embed = new Discord.RichEmbed()
                          .setColor("36393f")
                         .setDescription('**انتا تحتاج الي كتابة ._.pl clear دون اتباع الحجج**')
                      .setTimestamp()// By : Kahrba. || تم التطوير من قبل كههربا
                     .setFooter(bot.user.username+" ||", bot.user.avatarURL)
                     message.channel.send({embed});
				}// By : Kahrba. || تم التطوير من قبل كههربا
				...
				songsQueue = tempA.concat(shuffle(tempB));
						message.reply(` `);// By : Kahrba. || تم التطوير من قبل كههربا
                             const embed = new Discord.RichEmbed()
                          .setColor("36393f")// By : Kahrba. || تم التطوير من قبل كههربا
                         .setDescription('**تـم تبديل قائمة التشغيل اكتب ._.pl لمشاهدة قائمة الشتغيل الجديده**')
                      .setTimestamp()
			...
			break;
// By : Kahrba. || تم التطوير من قبل كههربا
		case "r":// By : Kahrba. || تم التطوير من قبل كههربا
			if (isPlaying) {
		...
		case "yt":
			if (args.length == 0) {// By : Kahrba. || تم التطوير من قبل كههربا
				...
				message.channel.send({embed: {
				color: 3447003,
				description: " 📝 || **__يجب عليك ادخال : ._.yt [ مصطلح البحث باليوتيوب]__**"
			}});
				...
				getYouTubeResultsId(args, 5).then(ytResults => {
					...
					let ytEmbed = new Discord.RichEmbed()
						...
						.addBlankField()
						.setFooter("شرح الاستخدام : ._.add [قم بوضع رقم البحث] ");
