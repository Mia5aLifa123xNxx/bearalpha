...
client.on('ready', () => {
 ...
 client.user.setActivity("",{type: '._.help'})
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
member.guild.fetchInvites().then(guildInvites => {
  ...
  message.guild.createChannel(`عدد المتواجدين في روم : [ ${message.guild.members.filter(m => m.voiceChannel).size} ]` , 'voice').then(c => {
    ...
    setInterval(() => {
      c.setName(`⟫『 ${message.guild.members.filter(m => m.voiceChannel).size} عدد اعضاء متواجدين في روم 』⟪`)
    },1000);
  });
  }
});
...
const fs = require("fs");
 const data = JSON.parse(fs.readFileSync('./data.json', 'utf8'));
  ...
  guild.members.get(ser.id).removeRoles(roles)
      ...
      fs.writeFile('./bearperson.json', JSON.stringify(bane), (err) => {
  if (err) console.error(err);
  })
...
client.on('message', message => {
    if(message.content.startsWith("._.toggleAntihack")) {
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');
